...
grouped_rates = all_investors_df.groupby('focus_classification')[['percent_250m_success', 'percent_100m_success', 'percent_25m_success']].mean()
print(grouped_rates)
'''
...
def calculate_balanced_outlier_score(row):
    ...
    # Outlier score
    outlier_score = W1 * adjusted_250m + W2 * adjusted_100m + W3 * adjusted_25m
...
# FEATURE 8: remove all 1-investment investors from the all_investors_df as they don't have any coinvestments, and then set all zero-outlier-score investors as REAL_WORLD_PROB (1.9 percent)
