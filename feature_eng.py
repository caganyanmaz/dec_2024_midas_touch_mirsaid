...
def main():
    ...
    training_investments_df = pd.read_csv(INPUT_INVESTMENTS_FILENAME)
    input_investors_df   = pd.read_csv(INVESTORS_FILENAME)
    testing_investors_df    = input_investors_df.copy()
    input_investors_df = extract_data_from_investments(input_investors_df, training_investments_df)
    ...
    best_picks = get_best_picks(input_investors_df, output_investments_df)
    # testing_investors_df = extract_data_from_investments(output_investors_df, output_investments_df)
    # create_features(output_investors_df)
    # print(get_investor_correlation(output_investors_df, output_investors_df))


def calculate_success_rates(investments):
    counters = {
        '25m': 0,
        '100m': 0,
        '250m': 0
    ...
    for investment in investments:
        ...
        if is_250m(investment):
            counters['250m'] += 1
    return counters['25m'] / len(investments), counters['100m'] / len(investments), counters['250m'] / len(investments) 
...
def get_best_picks(input_investor_df, output_investment_df):
    ...
    for investment in output_investment_df.itertuples():
        ...
        for investor in get_investors(investment):
            if investor in input_investor_df['investor_uuid']:
                current_value = input_investor_df[input_investor_df['investor_uuid'] == investor].at['weighted_outlier_rate']
                best_value = max(best_value, current_value)
        investment_value.append((best_value, investment))
    investment_value.sort(reverse=True)
    return list(map(lambda x: x[1], investment_value[:100]))
...
def add_earliest_investment_experiences(investors_df, investments_df, investor_columns):
    earliest_investment_experiences = [INPUT_OUTPUT_SPLIT_YEAR] * len(investors_df.index)
    for investment in investments_df.itertuples():
        for investor in get_investors(investment, investor_columns):
